...
export default function Comisiones() {
  ...
  const activasCon = operations.filter(
    (o) =>
      o.status === "ACTIVA" && ((o.compPct ?? 0) > 0 || (o.compFixed ?? 0) > 0),
  );
    ...
    <>
      <div className="grid grid-cols-3 gap-3.5 mb-6">
        <div className="bg-surface border border-border rounded-[10px] p-[18px]">
          <div className="text-[12px] text-text-2 mb-1.5 font-medium">
            Comisiones activas
          </div>
          <div className="text-[28px] font-bold text-gold-dark">
            {activasCon.length}
          </div>
          <div className="text-[11px] text-text-3 mt-0.5">
            Operaciones con comp.
          </div>
        </div>
        <div className="bg-surface border border-border rounded-[10px] p-[18px]">
          <div className="text-[12px] text-text-2 mb-1.5 font-medium">
      ...
      {byAgent.map((ag) => {
        ...
        return (
          <div
            className="bg-surface border border-border rounded-[10px] overflow-hidden mb-5"
            key={ag.id}
          >
            ...
            <div className="flex flex-wrap gap-3 p-[18px]">
              {ag.ops.map((op) => {
                const comm =
                  (op.compFixed ?? 0) > 0
                    ? (op.compFixed ?? 0)
                    : (op.price * (op.compPct || 0)) / 100;
                return (
                  <div
                    key={op.id}
                    className="flex-1 bg-bg border border-border rounded-[6px] px-3.5 py-3 min-w-[160px]"
                  >
                    <div className="text-[11px] text-text-3 font-semibold uppercase mb-1">
                      {op.address}
                    </div>
                    <div className="text-[20px] font-bold text-gold-dark">
                      {fmtPrice(comm)}
                    </div>
                    <div className="text-[11px] text-text-3 mt-0.5">
                      {(op.compFixed ?? 0) > 0
                        ? "Fijo"
                        : `${op.compPct}% de ${fmtPrice(op.price)}`}
                    </div>
                  </div>
                );
              })}
